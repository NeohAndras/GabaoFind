...
export async function loadListings(categoryFilter = null, searchQuery = '') {
  try {
    const listingsRef = collection(db, 'listings');
    
    // Build query
    let q = query(listingsRef, orderBy('name', 'asc'));
    
    if (categoryFilter && categoryFilter !== 'all') {
      q = query(listingsRef, where('category', '==', categoryFilter), orderBy('name', 'asc'));
    }
    ...
    // Apply client-side search (Firestore doesn't support full-text search on free tier)
    if (searchQuery.trim()) {
    ...
    renderListings(filteredListings); // Your existing render function
    updateStats(filteredListings.length); // Your existing stats function
    
    return filteredListings;
    
  } catch (error) {
    console.error('❌ Error loading listings:', error);
    showError('Failed to load directory. Please refresh.');
    return [];
  }
}
...
export function setupSearch() {
  ...
  searchInput?.addEventListener('input', (e) => {
    ...
    timeout = setTimeout(() => {
      loadListings(currentCategory, e.target.value);
    }, 300);
  });
}
...
export function setupCategoryFilter() {
  ...
  filters.forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      loadListings(category, document.getElementById('search-input')?.value || '');
    });
  });
}
